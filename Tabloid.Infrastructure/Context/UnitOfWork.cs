...
public class UnitOfWork<TId> : IUnitOfWork<TId>
{
    ...
    public TRepository GetRepository<TRepository>()
    {
        ...
        if (_repositories.TryGetValue(interfaceType, out Type? value))
        {
            return (TRepository)Activator.CreateInstance(value, _context)!;
        }
    ...
    private static IList<Type> GetImplementations(Assembly assembly)
    {
        ...
        var types = assembly
            .GetLoadableTypes()
            .Where(x => x is not null && x.IsClass && !x.IsAbstract && x.BaseType?.Name == baseClass.Name)
            .Select(x => x!)
            .OrderBy(x => x.Name)
            .ToList();
    ...
    private static IDictionary<Type, Type> CreateRepositoryDictionary(IList<Type> interfaces, IList<Type> implementations)
    {
        ...
        else if (!CheckPairings(interfaces, implementations))
        {
            throw new ArgumentException(
                "Every item in 'implementations' must implement the corresponding item in 'interfaces'.",
